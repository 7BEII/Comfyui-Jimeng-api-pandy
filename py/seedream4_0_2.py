...
class Seedream40_2:
    ...
    def get_dimensions_from_ratio_and_resolution(self, aspect_ratio, resolution):
        """根据比例和分辨率计算具体的宽高尺寸"""
        ...
        # 获取目标像素数和比例
        target_pixels = pixel_counts[resolution]
        ratio_w, ratio_h = ratios[aspect_ratio]
        
        # 计算实际尺寸
        # 设宽度为 ratio_w * k，高度为 ratio_h * k
        # 则总像素 = (ratio_w * k) * (ratio_h * k) = ratio_w * ratio_h * k²
        # 所以 k = sqrt(target_pixels / (ratio_w * ratio_h))
        k = (target_pixels / (ratio_w * ratio_h)) ** 0.5
    ...
    def format_size_for_api(self, width, height):
        """将宽高格式化为API接受的格式"""
        # 确保尺寸是64的倍数
        width = ((width + 63) // 64) * 64
        height = ((height + 63) // 64) * 64
...
模型：doubao-seedream-4-0-250828
"""  
    def generate_images(self, prompt, api_key, batch_size, aspect_ratio, resolution, guidance_scale,
        ...
        # 根据比例和分辨率计算具体尺寸
        width, height = self.get_dimensions_from_ratio_and_resolution(aspect_ratio, resolution)
        size_str = f"{width}x{height}"
        
        print(f"🚀 开始生成图像...")
        print(f"📝 提示词: {prompt[:100]}{'...' if len(prompt) > 100 else ''}")
        print(f"🤖 模型: {model_id}")
        print(f"📐 比例: {aspect_ratio}, 分辨率: {resolution} ({width}x{height})")
...
# 节点注册
NODE_CLASS_MAPPINGS = {
    "JM:Seedream 4.0_2": Seedream40_2
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "JM:Seedream 4.0_2": "JM:Seedream 4.0_2"
}
